...
int Instruction::GetLength(uint16_t address, SymbolTable &symbolTable) const
{
	std::shared_ptr<Symbol> symbolP1 = nullptr;

	if (operand_length > 0) {
		symbolP1 = symbolTable.GetSymbol(address + 1);
	}
	if(symbolP1) {
		return 1;
	}
	return 1 + operand_length;
}
...
{
	uint16_t opWord = op1 | (op2 << 8);

	auto symbol = symbolTable.GetSymbol(address);
	std::shared_ptr<Symbol> symbolP1 = nullptr;

	if (operand_length > 0) {
		symbolP1 = symbolTable.GetSymbol(address + 1);
	}
	// at this point, if symbolP1 is non-null, it means
	// the address contains a "skip" instruction to skip
	// the instruction at the immediately folloing label.

	std::string prefix;	// address :0x4, code (1, 2 or 3 bytes), label

	prefix = std::format("{:04X}: {:02X}", address, opcode);

	if (symbolP1) {
		// Our instruction crosses a label, so we're a "skip next
		// instruction" hack
		prefix += std::format("       ");
	} else {
		switch(operand_length) {
			case 0:
				...
				break;
			case 1:
				prefix += std::format(" {:02X}    ", op1);
				break;
			case 2:
				prefix += std::format(" {:02X} {:02X} ", op1, op2);
				break;
		}
	}

	std::string label;
	if (symbol) label = symbol->label + ":";

	prefix += label;
	int maxLabel = std::min(size_t(8), label.size());
	for (int i = 0; i < 8 - maxLabel; i++) {
		prefix += " ";
	}

	if (symbolP1) {
		if(mnemonic=="RET" || mnemonic == "PCHL") {
			return prefix + mnemonic;
		} else {
			return prefix + std::format("DB      {:02X}H", opcode);
		}
	} else {
		switch(operand_type) {
			case IMMEDIATE: {
					return prefix + std::format("{}{:02X}H", mnemonic, op1);
				}
				break;
			case IMMEDIATE_HYBRID:
			case ADDRESS: {
					auto opSymbol = symbolTable.GetSymbol(opWord);
					if (!opSymbol && setSymbol) {
						char t = '-';
						if (instruction_type == MOVE) t = 'C';
						if (instruction_type == DATA) t = 'D';
						symbolTable.SetSymbol(std::format("L{:04X}", opWord), opWord, t, 1 /* length */);
						opSymbol = symbolTable.GetSymbol(opWord);
					}
					if (opSymbol && opWord) {
						return prefix + std::format("{}{}", mnemonic, opSymbol->label);
					} else {
						return prefix + std::format("{}{:04X}H", mnemonic, opWord);
					}
