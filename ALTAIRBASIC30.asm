...
DSUB:				 ;NEGATE THE SECOND ARGUMENT
	LXI	H,ARG-1		;GET POINTER TO ARG SIGN
	MOV	A,M		;GET SIGN
	XRI	128		;COMPLEMENT SIGN BIT
	MOV	M,A		;SAVE IT
				;FALL INTO DADD
...
DADD2:
	...
	JZ	OVERR		;CHECK FOR OVERFLOW
	CALL	DSHFR1		;SHIFT NUMBER RIGHT ONE, SHIFT IN CARRY
...
DADD3:
	...
	LXI	H,FAC+1		;POINT TO THE UNPACKED SIGN
...
DRONA1:
	...
	;ALTERS A,C,D,E,H,L
DADDD:
	LXI	D,FBUFFR+17		;ENTRY FROM DDIV
	LXI	H,ARGLO		;ADD OR SUBTRACT FBUFFR+17 AND ARG
...
DADDOP:
	...
	;ALTERS A,B,C,H,L
DNEGR:
	MOV	A,M		;COMPLEMENT SIGN OF FAC
...
DNEGR1:
	...
	;SHIFT DBL FAC OR ARG RIGHT ONE
	;A = SHIFT COUNT
	;ALTERS A,C,D,E,H,L
DSHFTR:
	MOV	M,C		; ?
	PUSH	H		; SAVE FAC OR ARG PTR FOR LATER USE
DSHFT1:
...
DMULT3:
	...
	CALL	DSHFR1		;ROTATE PRODUCT RIGHT ONE
...
;	CALL	VMOVFM		;MOVE TEN INTO THE FAC
	LXI	D,DTEN		;DBL 10.0
	LXI	H,ARGLO
	CALL	VMOVE		;LOAD DBL 10.0 INTO ARG
...
DDIV:
	...
	INR	M
	CALL	DMULDV		;ZERO FAC AND PUT FAC IN FBUFFR
	LXI	H,FBUFFR+24		;GET POINTER TO THE EXTRA HO BYTE ABOVE FBUFFR
	MOV	M,C		;ZERO IT
...
DDIV2:
	...
	LXI	H,FBUFFR+17		;GET POINTER TO LO IN FBUFFR
...
FINDIV:
	PUSH	D		;SAVE (DE) FOR FOUT CALLERS
	PUSH	H		;SAVE (HL) FOR FOUT CALLERS
...
FOUTCB:
	...
	ENDIF
	;CONSTANTS FOR FOUT
	;DBL NORMALIZATION UPPER BOUND (9999999999999999.5)
DBLUBND:
	DB	253,255,159,49,169,95,99,178
	;DBL NORMALIZATION LOWER BOUND (999999999999999.95)
DBLLBND:
	DB	254,255,3,191,201,27,14,182
	;DBL 10^10 CONSTANT FOR FOUTND NORMALIZATION LOOP
DTEN10:
	DB	0,0,0,0,249,2,21,162
	;DBL 1E16 CONSTANT FOR FOUTCV ROUNDING
DBLE16:
	DB	0,0,0,0,0,0,0,0
	;DBL 0.5 -- FHALF-4 IS THE DBL VERSION
DFHALF:
	DB	0,0,0,0
FHALF:
	DB	0		;1/2
	...
	DB	0
	DB	128
	;DBL POWERS-OF-TEN TABLE FOR FOUTCV (10 ENTRIES, 7 BYTES EACH)
	;10^15 THROUGH 10^6
DBLTBL:
	DB	0,128,198,164,126,141,3		;10^15
	DB	0,64,122,16,243,90,0		;10^14
	DB	0,160,114,78,24,9,0		;10^13
	DB	0,16,165,212,232,0,0		;10^12
	DB	0,232,118,72,23,0,0		;10^11
	DB	0,228,11,84,2,0,0		;10^10
	DB	0,202,154,59,0,0,0		;10^9
	DB	0,225,245,5,0,0,0		;10^8
	DB	128,150,152,0,0,0,0		;10^7
	DB	64,66,15,0,0,0,0		;10^6
	;POWER OF TEN TABLE (SNG, 3 BYTES EACH)
FOUTBL:
	DB	160		; 100000
	DB	134
	DB	1
...
FOUTNV:
	PUSH	D		;SAVE (DE)
	XRA	A		;ZERO THE DECIMAL EXPONENT
	PUSH	PSW		;SAVE IT
	CALL	GETYPE		;CHECK WHAT TYPE WE HAVE
	JNC	FOUTND		;WE HAVE A DBL, GO NORMALIZE IT
	;SNG OR AFTER DBL COARSE NORMALIZATION: FINE NORMALIZE
FONSCL:
	CALL	FOUNSC		;IS THE FAC TOO BIG OR TOO SMALL?
FOUNS1:
	CALL	GETYPE		;CHECK TYPE FOR COMPARISON
	JNC	FONS1D		;DBL -- USE DCOMPD
	;SNG COMPARISON
	MOVRI	145,67,79,248	;GET 99999.9499 TO SEE IF THE FAC IS BIG
	CALL	FCOMP		; ENOUGH YET
	JMP	FONS1C		;GO TO COMMON PATH
FONS1D:
	;DBL COMPARISON
	LXI	D,DBLUBND	;GET 9999999999999999.5
	CALL	DCOMPD		;COMPARE FAC WITH DBL UPPER BOUND
FONS1C:
	JP	FOUNS3		;IT IS BIG ENOUGH, WE ARE DONE
...
FOUNSC:
	CALL	GETYPE		;CHECK TYPE FOR COMPARISON
	JNC	FONSCD		;DBL -- USE DCOMPD
	;SNG COMPARISON
	MOVRI	148,116,35,247	;GET 999999.499 TO SEE IF THE FAC IS TOO BIG
	CALL	FCOMP
	JMP	FONSCC		;GO TO COMMON PATH
FONSCD:
	;DBL COMPARISON
	LXI	D,DBLLBND	;GET 999999999999999.95
	CALL	DCOMPD		;COMPARE FAC WITH DBL LOWER BOUND
FONSCC:
	POP	H		;GET THE RETURN ADRESS OFF THE STACK
	JP	FOUNS2		;IT IS TOO BIG, MAKE IT SMALLER
	PCHL			;IT IS SMALL ENOUGH, RETURN
	;HERE TO NORMALIZE A DBL NUMBER

	;HERE TO NORMALIZE A DBL NUMBER FOR FOUT
	;MULTIPLY BY 10^10 REPEATEDLY UNTIL FAC EXPONENT >= 91H
	;THEN FALL INTO FOUNS1 FOR FINE-GRAINED NORMALIZATION
FOUTND:
	LDA	FAC		;GET THE EXPONENT
	CPI	91H		;IS IT BIG ENOUGH?
	JNC	FONSCL		;YES, CHECK IF TOO BIG, THEN FINE NORMALIZE
	LXI	D,DTEN10	;GET POINTER TO DOUBLE PRECISION 10^10
	LXI	H,ARGLO		;LOAD 10^10 INTO ARG
	CALL	VMOVE
	CALL	DMULT		;MULTIPLY FAC BY 10^10
	POP	PSW		;GET THE DECIMAL EXPONENT
	SUI	10		;SUBTRACT 10 (WE MULTIPLIED BY 10^10)
	PUSH	PSW		;SAVE UPDATED EXPONENT
	JMP	FOUTND		;CHECK AGAIN

	;HERE TO PRINT A NUMBER IN FIXED FORMAT
	;ENTERED FROM FOUT2 WITH VALTYP IN A, FORMAT SPECS IN D
FOUTFX:
	INX	H		;POINT PAST THE ZERO
	PUSH	B		;SAVE THE FIELD LENGTH SPECS
	JMP	FOUTFC		;GO TO THE FIXED FORMAT HANDLER
...
L356D:	RZ
	CALL	FOUTDP
	;PRINT TRAILING ZEROS WITH COMMA/DECIMAL POINT HANDLING
FOTZRC:
L3571:	MVI	M,'0'
	INX	H
	DCR	A
	...
	;CONVERT THE FAC TO DECIMAL DIGITS IN THE BUFFER
...
FOUTCV:
	CALL	GETYPE		;CHECK IF SNG OR DBL
	JNC	FOCVDB		;DBL -- GO TO DBL PATH
	;SNG PATH: ADD 0.5 FOR ROUNDING, CONVERT TO INTEGER, EXTRACT 6 DIGITS
	...
	JMP	FOCVS6		;GO EXTRACT 6 SNG DIGITS
	;DBL PATH: EXTRACT 10 DIGITS WITH 7-BYTE TABLE, THEN 6 MORE VIA SNG
FOCVDB:
	PUSH	B		;SAVE COUNTERS
	PUSH	H		;SAVE BUFFER POINTER
	CALL	VMOVAF		;MOVE FAC TO ARG
	LXI	H,DBLE16	;LOAD DBL ROUNDING CONSTANT INTO FAC
	CALL	VMOVFM		;(THIS IS ZERO, EFFECTIVELY NO ROUNDING FOR DBL)
	CALL	DADD		;ADD ROUNDING CONSTANT
	XRA	A		;CLEAR CARRY FOR DINTFO (DON'T RE-NORMALIZE)
	CALL	DINTFO		;CONVERT DBL FAC TO INTEGER FORM
	POP	H		;RESTORE BUFFER POINTER
	POP	B		;RESTORE COUNTERS
	LXI	D,DBLTBL	;POINT TO 7-BYTE DBL POWERS-OF-10 TABLE
	MVI	A,10		;10 DBL DIGITS TO EXTRACT
	;DBL DIGIT EXTRACTION LOOP
FOCVD1:
	...
	MVI	B,'0'-1		;INITIALIZE DIGIT ASCII
FOCVD2:
	INR	B		;INCREMENT DIGIT
	POP	H		;GET TABLE POINTER (DADDFO NEEDS IT IN HL)
	PUSH	H		;SAVE IT AGAIN
	MVI	A,9EH		;SBB M OPCODE -- SUBTRACT
	CALL	DADDFO		;SUBTRACT 7-BYTE POWER OF 10 FROM DFACLO
	JNC	FOCVD2		;KEEP GOING IF RESULT WAS POSITIVE
	;WENT ONE TOO FAR, ADD THE POWER OF 10 BACK
	POP	H		;GET TABLE POINTER
	PUSH	H		;SAVE IT AGAIN
	MVI	A,8EH		;ADC M OPCODE -- ADD
	CALL	DADDFO		;ADD 7-BYTE POWER OF 10 BACK TO DFACLO
	;DADDFO ADVANCED HL BY 7, SO HL NOW POINTS PAST THIS ENTRY
	;BUT WE STILL HAVE THE OLD TABLE POINTER ON THE STACK
	XCHG			;DE = NEXT TABLE ENTRY (HL AFTER DADDFO)
	POP	H		;DISCARD OLD TABLE POINTER
	POP	H		;GET BUFFER POINTER
	MOV	M,B		;PUT ASCII DIGIT IN BUFFER
	INX	H		;INCREMENT BUFFER POINTER
	POP	PSW		;GET DIGIT COUNT
	POP	B		;GET COUNTERS
	DCR	A		;MORE DBL DIGITS?
	JNZ	FOCVD1		;YES, DO NEXT DIGIT
	;10 DBL DIGITS DONE, LOAD REMAINDER AS SNG
	PUSH	B		;SAVE COUNTERS
	PUSH	H		;SAVE BUFFER POINTER
	LXI	H,DFACLO	;GET POINTER TO LOW 4 BYTES OF DBL FAC
	CALL	MOVFM		;LOAD INTO FAC AS SNG VALUE
	POP	H		;RESTORE BUFFER POINTER
	POP	B		;RESTORE COUNTERS
	;FALL INTO SNG 6-DIGIT EXTRACTION FOR REMAINING DIGITS
FOCVS6:
...
FFXDXM:	DB	0,0,4,191,201,27,14,182	;1E16 DOUBLE PRECISION
